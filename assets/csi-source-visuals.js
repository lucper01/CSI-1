(() => {
  ...
  style.textContent = `
    ...
    .csi-source-visual{position:absolute;z-index:0;pointer-events:none;margin:0;overflow:hidden;opacity:var(--csi-vis-opacity,.26);filter:saturate(.84) contrast(.97);transition:opacity .2s ease}
    ...
    .csi-source-visual.anatomy.soft{--csi-vis-opacity:.16}
    .csi-source-visual.photo{width:clamp(210px,22vw,330px);height:clamp(125px,14vw,200px);right:clamp(28px,3.5vw,62px);bottom:clamp(62px,7vh,94px);border-radius:24px;box-shadow:0 14px 34px rgba(11,47,35,.12);border:1px solid rgba(255,255,255,.62);--csi-vis-opacity:.24}
    .csi-source-visual.photo img{object-fit:cover}
    .csi-source-visual.duo-a{right:clamp(135px,12vw,205px);bottom:clamp(64px,7vh,92px);width:clamp(118px,12vw,175px);height:clamp(105px,11vw,155px);--csi-vis-opacity:.16}
    .csi-source-visual.duo-b{right:clamp(28px,3vw,55px);bottom:clamp(84px,9vh,118px);width:clamp(118px,12vw,175px);height:clamp(105px,11vw,155px);--csi-vis-opacity:.16}
    .csi-source-visual.study-photo{width:clamp(190px,19vw,290px);height:clamp(112px,12vw,172px);right:clamp(26px,3vw,54px);top:clamp(112px,15vh,154px);bottom:auto;--csi-vis-opacity:.22}
    .csi-source-visual.activity-photo{width:clamp(210px,21vw,320px);height:clamp(130px,14vw,205px);right:clamp(26px,3vw,56px);bottom:clamp(62px,7vh,90px);--csi-vis-opacity:.20}
    .csi-source-visual.activity-mic{width:clamp(110px,12vw,170px);height:clamp(100px,11vw,160px);right:clamp(205px,18vw,310px);bottom:clamp(70px,8vh,105px);--csi-vis-opacity:.14}
    ...
    html[data-theme="dark"] .csi-source-visual.photo{filter:saturate(.65) brightness(.78)}
    body.high-contrast .csi-source-visual{display:none!important}
    @media(max-width:900px){.csi-source-visual{opacity:.10!important}.csi-source-visual.photo{width:180px;height:110px}.csi-source-visual.anatomy{width:145px;height:125px}}
    @media print{.csi-source-visual{opacity:.16!important}}
  `;
